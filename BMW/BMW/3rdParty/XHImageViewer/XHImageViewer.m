...
- (void)showWithSelectedView:(UIImageView *)selectedView {
                         ...
                         for (UIImageView *view in _imgViews) {
                             ...
                             XHZoomingImageView *tmp = [[XHZoomingImageView alloc]
                                                        initWithFrame:CGRectMake([_imgViews indexOfObject:view] * fullW,
                                                                                 0, fullW, fullH)];
                             tmp.imageView = view;
                             tmp.imageView.contentMode = UIViewContentModeScaleToFill;
    ...
    // Appending By Leo
    if ([self.delegate respondsToSelector:@selector(imageViewer:didShowImageView:)]) {
        [self.delegate imageViewer:self didShowImageView:selectedView];
    }
}
...
#pragma mark - Public methods By Leo

- (void)disMissWithSelectedView {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    // Appending By Leo
    if ([self.delegate respondsToSelector:@selector(imageViewer:didShowImageView:)]) {
        [self.delegate imageViewer:self didShowImageView:[self currentView]];
    }
}
